...
class ProgressCircle {
	constructor(options) {
        ...
        this.elements = { titleLabel: null, timeLabel: null, centerCircle: null };
		this.content = this.generateContent();
        this.applyOptions(this.options);
	}
	
	generateContent() {
        if (!this.options || !this.options.time) { console.error("When creating a ProgressCircle, 'time' value must be supplied."); return null; }
        this.currentTime = this.options.time;
        this.maxTime = this.options.time;
        this.startTime = (new Date()).getTime();

		let container = new Container(this.options);

        this.elements.centerCircle = new Container({
            style: {
                position: "absolute",
                top: "0px",
                width: "200px",
        ...
        container.appendChild(this.elements.centerCircle.content);

        let progressContainer = new Container({})
        this.circleProgress = new CircleProgress(progressContainer.content, {
            max: this.options.time,
            value: this.options.time,
            animationDuration: 400,
            textFormat: (value, max) => { return ""; },
        });
        container.appendChild(progressContainer.content);

        this.elements.titleLabel = new Label({
            style: {
                position: "absolute",
                top: "-10px",
                left: "0px",
            ...
            attributes: { value: this.options.title ? this.options.title : "" }
        });
        container.appendChild(this.elements.titleLabel.content);

        this.elements.timeLabel = new Label({
            style: {
                position: "absolute",
                top: "30px",
                left: "0px",
        ...
        container.appendChild(this.elements.timeLabel.content);
    ...
    updateCurrentTime() {
        ...
        let diff = Math.floor(now - this.startTime);
        this.currentTime = this.maxTime - parseInt(diff / 1000);

        this.circleProgress.value = this.currentTime;
        this.elements.timeLabel.setValue(this.getTimeString());
    }

    getTimeString() {
        if (this.currentTime < 10) { return "0" + this.currentTime.toString(); }
        else if (this.currentTime < 60) { return this.currentTime.toString(); }
        else {
            let seconds = (this.currentTime % 60);
            seconds = (seconds < 10) ? ("0" + seconds.toString()) : seconds.toString();
            return (parseInt(this.currentTime / 60)).toString() + ":" + seconds;
        }
    }
